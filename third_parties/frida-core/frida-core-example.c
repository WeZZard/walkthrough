 ...
 * Compile with:
 *
 * clang  frida-core-example.c -o frida-core-example -L. -lfrida-core -lbsm -ldl -lm -lresolv -Wl,-framework,Foundation,-framework,AppKit
 *
      ...
      char * argv[])
{
  ...
  session = frida_device_attach_sync (local_device, target_pid, options, NULL, &error);
