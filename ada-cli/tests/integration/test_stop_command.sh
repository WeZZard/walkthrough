#!/bin/bash
#
# Test: Using 'ada capture stop' command
#
# Scenario: Start capture in background, then use stop command.
# Expected: Session completes gracefully.
#
...
# Give it time to start and register session
sleep 3

# Use stop command
echo "Running 'ada capture stop'..."
"$ADA" capture stop || {
    echo "Stop command failed, cleaning up..."
...
if ! wait_for_process_exit $ADA_PID 30; then
    kill -9 $ADA_PID 2>/dev/null || true
    fail "ada did not exit within timeout after stop command"
fi
